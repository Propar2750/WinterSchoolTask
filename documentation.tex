...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\columnwidth]{task1/processing_demo/output/road_mask.png}
\caption{Final binary road mask after the complete preprocessing pipeline.}
\label{fig:road_mask}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\columnwidth]{task1/rrt_demo/output/osm_road_mask.png}
\caption{Road mask extracted from the OpenStreetMap tile of IIT Kharagpur campus.}
\label{fig:osm_road_mask}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=\columnwidth]{task1/rrt_demo/output/rrt_star_result.jpg}
\caption{RRT* result on the 800$\times$800 OSM-based map, reaching the goal at iteration $\sim$35{,}221 with a path cost of $\sim$1236\,px.}
\label{fig:osm_rrt_result}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\columnwidth]{task2/output/segmented_clusters.png}
\caption{K-Means segmented clusters ($K=3$). Each cluster is visualized with its centroid color, showing how the algorithm partitions the image by color similarity.}
\label{fig:segmented_clusters}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\columnwidth]{task2/output/foreground_mask.png}
\caption{Foreground mask generated by K-Means background identification and morphological cleanup. White regions indicate detected foreground.}
\label{fig:fg_mask}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.6\columnwidth]{task2/output/result.png}
\caption{Final composite result with the background replaced by a stock image using Gaussian-feathered alpha blending.}
\label{fig:result}
\end{figure}
...
\caption{Task 2 pipeline overview: (left to right) original image, K-Means segmented clusters, replacement background, final composite result.}
